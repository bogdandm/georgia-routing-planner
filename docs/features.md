...
single flat list with one shared opacity slider. The slider scales the five controlled
OpenStreetMap feature families together while preserving their relative visual weights
and individual visibility choices. Every map data source added to the application must
...
never cross the UI boundary. Satellite controls remain disabled until a scene is
applied. Hiding imagery retains the applied scene and does not remove its footprint,
search results, or attribution contract. Relief and isoline visibility are independent
of 3D terrain mode and satellite availability. Base land remains visible and cannot be
