...
function markerUpsertRequest(overrides: Record<string, unknown> = {}): Request {
  return jsonRequest({
    action: 'marker-upsert',
    markerId: 'marker-a',
    baseRevision: 0,
...
Deno.test(
  ...
  async () => {
    ...
    state.rpcResults.set('upsert_marker', [
      { data: { outcome: 'conflict', record }, error: null },
    ]);
    state.rpcResults.set('delete_marker', [
      { data: { outcome: 'missing' }, error: null },
      { data: { outcome: 'applied', record }, error: null },
    ]);

    const conflict = await handleTrackSync(markerUpsertRequest(), makeContext(state));
    assertEquals(await responseJson(conflict), { outcome: 'conflict', record });

    const missing = await handleTrackSync(
      jsonRequest({ action: 'marker-delete', markerId: 'marker-a', baseRevision: 3 }),
      makeContext(state),
    );
    assertEquals(await responseJson(missing), { outcome: 'missing' });

    const applied = await handleTrackSync(
      jsonRequest({ action: 'marker-delete', markerId: 'marker-a', baseRevision: 3 }),
      makeContext(state),
    );
    assertEquals(await responseJson(applied), { outcome: 'applied', record });
    assertEquals(
      state.calls
      ...
      [
        ...
        [
          'delete_marker',
          { p_user_id: USER_ID, p_marker_id: 'marker-a', p_base_revision: 3 },
        ],
        [
          'delete_marker',
          { p_user_id: USER_ID, p_marker_id: 'marker-a', p_base_revision: 3 },
...
Deno.test('marker revision exhaustion is a bounded conflict response', async () => {
  const state = makeState();
  state.rpcResults.set('delete_marker', [
    { data: { outcome: 'revision-exhausted' }, error: null },
  ]);
  const response = await handleTrackSync(
    jsonRequest({ action: 'marker-delete', markerId: 'marker-a', baseRevision: 1 }),
    makeContext(state),
  );
  assertEquals(response.status, 409);
  assertEquals((await responseJson(response)).error, {
    code: 'marker_revision_exhausted',
    message: 'Marker revisions are exhausted.',
  });
});

Deno.test('invalid marker commands never call the RPC boundary', async () => {
  ...
  const cases: readonly unknown[] = [
    ...
    },
    {
      ...
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, name: ' Marker', normalizedName: ' marker' },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, normalizedName: 'Marker' },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, iconKey: 'unsupported' },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, colorKey: 'unsupported' },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, coordinate: [181, 41.7] },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, updatedAt: 'not-a-date' },
    },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
      baseRevision: 0,
      marker: { ...valid, id: 'marker-b' },
    },
    { action: 'marker-upsert', markerId: 'marker-a', baseRevision: -1, marker: valid },
    {
      action: 'marker-upsert',
      markerId: 'marker-a',
    ...
    },
    {
      ...
      marker: {
        ...valid,
        name: 'a'.repeat(4_097),
        normalizedName: 'a'.repeat(4_097),
      },
    },
  ];
  for (const value of cases) {
    const state = makeState();
    const response = await handleTrackSync(jsonRequest(value), makeContext(state));
    assertEquals(response.status, 400);
    assertEquals(
      state.calls.filter((call) => call.kind === 'rpc'),
      [],
    );
  }
});
